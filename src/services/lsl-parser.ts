...
export class LSLParser {
  ...
  static validateLSLCode(code: string): LSLValidationResult {
    ...
    if (hasDefaultState) confidence += 0.2;
    if (hasStateEntry) confidence += 0.1;
    if (tokens.some(t => t.type === 'function' && this.LSL_FUNCTIONS.includes(t.value))) {
      confidence += 0.2;
    }
    
    // Reduce confidence for contamination
    if (hasPHPTags || hasHTMLTags || hasMediaWikiSyntax) confidence -= 0.5;
  ...
  static extractLSLFunction($: CheerioAPI): Partial<LSLFunction> {
    const func: Partial<LSLFunction> = {};
