...
const ossl_functions_1 = require("../data/ossl-functions");
class CoverageReport {
    static generateReport() {
        const functions = ossl_functions_1.OSSLFunctions.getAllFunctions();
        const categories = ossl_functions_1.OSSLFunctions.getCategories();
        ...
        const categoryStats = categories.map(category => {
            const categoryFunctions = ossl_functions_1.OSSLFunctions.getFunctionsByCategory(category);
            return {
                category,
        ...
        const criticalScore = (criticalFunctions.filter(name => functions.find(f => f.name === name)).length / criticalFunctions.length) * 40; // Max 40 points for critical functions
