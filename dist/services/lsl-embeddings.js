...
exports.LSLEmbeddingsService = void 0;
const lsl_parser_js_1 = require("./lsl-parser.js");
class LSLEmbeddingsService {
    static VECTOR_DIMENSIONS = 128;
    functionEmbeddings = new Map();
    patternEmbeddings = new Map();
    constructor() {
        this.initializeBasicEmbeddings();
    }
    initializeBasicEmbeddings() {
    ...
    inferReturnType(functionName) {
        ...
        if (name.includes('get') && name.includes('pos'))
            return 'vector';
        if (name.includes('get') && name.includes('rot'))
            return 'rotation';
        if (name.includes('get') && name.includes('owner'))
            return 'key';
        if (name.includes('get') && name.includes('name'))
            return 'string';
        if (name.includes('listen'))
            return 'integer';
        if (name.includes('say') || name.includes('whisper') || name.includes('shout'))
            return 'void';
    ...
    findSimilarFunctions(functionName, maxResults = 5) {
    ...
    findSimilarByName(functionName, maxResults = 5) {
        const results = [];
    ...
    findFunctionsByPattern(pattern) {
    ...
    calculateNameSimilarity(name1, name2) {
        const tokens1 = lsl_parser_js_1.LSLParser.tokenize(name1).filter(t => t.type !== 'whitespace');
        const tokens2 = lsl_parser_js_1.LSLParser.tokenize(name2).filter(t => t.type !== 'whitespace');
    ...
    explainSimilarity(meta1, meta2) {
        ...
        const commonParams = meta1.parameters.filter((p) => meta2.parameters.includes(p));
    ...
    addCustomFunction(name, category, complexity, params) {
...
exports.LSLEmbeddingsService = LSLEmbeddingsService;
//# sourceMappingURL=lsl-embeddings.js.map