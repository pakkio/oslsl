...
exports.LSLSecurityScanner = void 0;
class LSLSecurityScanner {
    threats = new Map();
    constructor() {
    ...
    detectThreat(threat, tokens, code, lines) {
        const vulnerabilities = [];
        try {
            switch (threat.id) {
    ...
    detectHttpInjection(threat, code, lines, vulnerabilities) {
    ...
    detectCommandInjection(threat, code, lines, vulnerabilities) {
    ...
    detectUnauthorizedPermissions(threat, code, lines, vulnerabilities) {
    ...
    detectPermissionWithoutHandler(threat, code, lines, vulnerabilities) {
    ...
    detectResourceExhaustion(threat, code, lines, vulnerabilities) {
        const patterns = [
    ...
    detectMemoryLeak(threat, code, lines, vulnerabilities) {
    ...
    detectSensitiveDataExposure(threat, code, lines, vulnerabilities) {
    ...
    detectDebugInformationLeak(threat, code, lines, vulnerabilities) {
    ...
    detectDenialOfService(threat, code, lines, vulnerabilities) {
        const spamPatterns = [
            /while\s*\([^)]*\)\s*\{[^}]*llEmail[^}]*\}/,
            /for\s*\([^)]*\)\s*\{[^}]*llSay[^}]*\}/,
    ...
    detectInfiniteLoop(threat, code, lines, vulnerabilities) {
    ...
    detectInsecureChannel(threat, code, lines, vulnerabilities) {
    ...
    detectBroadcastSensitive(threat, code, lines, vulnerabilities) {
    ...
    createVulnerability(threat, line, column, lines, evidence) {
        return {
            threatId: threat.id,
    ...
    generateSecurityReport(vulnerabilities) {
        ...
        const overallRisk = summary.critical > 0 ? 'critical' :
            summary.high > 0 ? 'high' :
    ...
    performComplianceChecks(vulnerabilities) {
        return {
...
exports.LSLSecurityScanner = LSLSecurityScanner;
//# sourceMappingURL=lsl-security-scanner.js.map