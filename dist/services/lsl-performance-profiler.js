...
exports.LSLPerformanceProfiler = void 0;
class LSLPerformanceProfiler {
    static EXECUTION_COSTS = {
    ...
    calculateMetrics(tokens, code) {
        const lines = code.split('\n');
        // Cyclomatic complexity
        const conditionals = tokens.filter(t => t.type === 'keyword' && ['if', 'else', 'while', 'for', 'do'].includes(t.value)).length;
        ...
        const eventHandlerCount = eventPatterns.filter(event => new RegExp(`\\b${event}\\s*\\(`).test(code)).length;
        // Variable declarations
        const variablePatterns = ['integer', 'float', 'string', 'key', 'vector', 'rotation', 'list'];
        const variableCount = variablePatterns.reduce((count, type) => count + (code.match(new RegExp(`\\b${type}\\s+\\w+`, 'g')) || []).length, 0);
        // Operations
    ...
    analyzeResourceUsage(tokens, code) {
        // Memory usage
        const typePatterns = {
        ...
        for (const [type, pattern] of Object.entries(typePatterns)) {
            ...
            const cost = LSLPerformanceProfiler.MEMORY_COSTS[type] || 0;
            memoryUsage += matches * cost;
            if (type === 'string')
                memoryCounts.strings += matches;
            else if (type === 'list')
                memoryCounts.lists += matches;
            else
                memoryCounts.variables += matches;
        }
    ...
    calculateExecutionCost(tokens) {
        let totalCost = 0;
        for (const token of tokens) {
            if (token.type === 'function') {
                const cost = LSLPerformanceProfiler.EXECUTION_COSTS[token.value];
                if (cost) {
                    totalCost += cost;
                }
                else {
    ...
    identifyBottlenecks(tokens, code, metrics) {
        const bottlenecks = [];
        // High complexity
    ...
    suggestOptimizations(tokens, code, metrics, bottlenecks) {
        const optimizations = [];
    ...
    calculateOverallScore(metrics, bottlenecks, resourceUsage) {
        let score = 100;
        // Complexity penalty
...
exports.LSLPerformanceProfiler = LSLPerformanceProfiler;
//# sourceMappingURL=lsl-performance-profiler.js.map